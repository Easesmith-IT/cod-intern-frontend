...
export const Mentor = ({ img,name, position,proficiency = [] }) => {
  return (
    <Card className="border-border-1 rounded-md">
      <CardContent>
        <Image src={img} width={458} height={396} className="object-cover max-w-[458px] w-full max-h-[396px] h-full" alt="mentor" />
            ...
            <>
              ...
              <div className="flex gap-2 flex-wrap items-center mt-2 capitalize">
                {proficiency.map((item, index) => (
                  <div
                    key={index}
                    className="bg-main rounded-md text-white py-1 px-1.5 text-sm font-stolzl font-normal"
                  >
                    {item}
                  </div>
                ))}
